...
class TimeRangeBuild:
    """
    This class is meant to take today's datetime convert it into 
    milliseconds. Then based on inputs into the class it determines the 
    millisecond time stamp for a date in the past. Ultimately it gives you 
    a datetime range that you can use as bounds for a query where the 
    ...
    def nTimeRangeBack(self):
        nBack_dt = self.now_dt_ - relativedelta(
            years=self.years_, months=self.months_, days=self.days_
        )
...
class APIOffset_UrlGather:
    """
    This class takes in an input of an API url taken from a given dataset 
    on the OpenBaltimore website: https://data.baltimorecity.gov/ 
    The functions of this class are able to modify the API urls and check 
    for the number of features in a GET request response. Combining these
    functions allow you to programatically generate a list of working urls 
    that you can use to obtain all the data in a given Open Baltimore 
    dataset.
    """
    ...
    def AddOffsetToURL(self, offset):
        ...
        new_queries_li = (
            [offsetAddition] + [paginationAddition] + self.queriesli_
        )
    ...
    def GatherOffsetUrls(self):
        """
        This function is used to programatically create all the urls 
        necessary to obtain the entirety of a given Open Baltimore dataset. 
        Each request to Open Baltimore is limited to 2000 "rowIDs" as 
        of 11/20/2023.
        """
