...
class BmoreDataExtraction:
    ...
    def LinkCheck(self, data_type_str):
        HTTP_Status_dict = {
            data_name: requests.get(
                self.API_dict_[data_type_str][data_name]
            ).status_code
            for data_name in self.API_dict_[data_type_str].keys()
        }
        available_li = [
            name if HTTP_Status_dict[name] == 200 else None
    ...
    def DownloadDataType(self, data_type_str, includeAll, chosen_dir=None):
        ...
        if includeAll is False:
            ...
            for data_name in tqdm(available_li):
                filename_str = (
                    OGName_CleanName_dict[data_name]
                    + "_"
                    + str(date.today())
                    + ".json"
                )
            ...
            for data_name in available_li:
                newSubPath_str = (
                    path_str + "/" + OGName_CleanName_dict[data_name] + "/"
                )
    ...
    def FeatureCollectionToDF(self, DataPath_dict):
        ...
        for key in tqdm(DataPath_dict.keys()):
            ...
            FEAT_DATA_DF_dict[key] = self.ExtractAllProperties(
                oneTableData_dict
            )
    ...
    def ExtractAllProperties(self, oneDataFile_dict):
        try:
            row_li = [
                sub_dict["properties"]
                for sub_dict in oneDataFile_dict["features"]
            ]
    ...
    def ExtractAllCoordinates(self, oneDataFile_dict):
        AREAgeo_ar_li = [
            np.array(AREA["geometry"]["coordinates"][0])
    ...
    def GeoDataToDF(self, DataPath_dict):
        ...
        for key in tqdm(DataPath_dict.keys()):
            ...
            GEO_DATA_df = properties_df.merge(
                meanGeos_df, left_index=True, right_index=True
            )
