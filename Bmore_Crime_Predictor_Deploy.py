...
deploy_ar = np.reshape(
    deploy_df.values, newshape=(1, deploy_shape_tu[0], deploy_shape_tu[1])
)

## Obtaining forecasted datetime range
LATEST_DATETIME = deploy_df.tail(1).index[0]
FORECAST_DATE_RANGE = pd.date_range(
    start=LATEST_DATETIME, freq="1H", periods=metadata_dict["OUT_STEPS"] + 1
)[1:]
Dlog.info(
    "Date Range of Prediction From "
    + str(LATEST_DATETIME)
    + " To "
    + str(FORECAST_DATE_RANGE[-1])
)
...
forecast_df = pd.DataFrame(
    norm.inverse_transform(pred_df),
    columns=pred_df.columns,
    index=pred_df.index,
)
forecast_df.to_csv(
    "Bmore_Crime_Forecast.csv", index=True, columns=forecast_df.columns
)
