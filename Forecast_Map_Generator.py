...
Bmore_Crime_Forecast_df.index = pd.to_datetime(Bmore_Crime_Forecast_df['Unnamed: 0'].rename('DATETIME'))
...
GEO_Crime_Forecast_df = Bmore_Crime_Forecast_df[metadata_dict['geo_cols_']+['Day sin']]
# GEO_Crime_Forecast_df = Bmore_Crime_Forecast_df[metadata_dict['geo_cols_']]
scaled_weights = mms.transform(Bmore_Crime_Forecast_df['Day sin'].values.reshape(-1, 1))
...
crime_forecast_map = folium.Map(location=[39.299236, -76.609383],
                                tiles='OpenStreetMap',
                                zoom_start=12)
